...
def update(sc, sc_input):
    ...
    if sc_input_previous == SCI_NULL:
        sc_input_previous = sc_input
        return

    pressed_right, pressed_left = False, False

    ptr_left = vptr.VirtualPointer(vkb.KeyState.INACTIVE, sc_adjust_x(sc_input.lpad_x, 1/4),
                                   sc_adjust_y(sc_input.lpad_y, 1/2))
    ptr_right = vptr.VirtualPointer(vkb.KeyState.INACTIVE, sc_adjust_x(sc_input.rpad_x, 3/4),
